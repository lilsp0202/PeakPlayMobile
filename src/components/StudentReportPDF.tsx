...
export default function StudentReportPDF({
  ...
  return (
    <motion.button
      ...
      onClick={generatePDF}
      disabled={isGenerating}
      className={`flex-1 px-3 py-4 sm:py-3 sm:px-2 sm:py-2 bg-gradient-to-r from-blue-600 to-purple-600 text-white text-sm font-medium rounded-xl sm:rounded-lg hover:from-blue-700 hover:to-purple-700 transition-all duration-200 flex items-center justify-center gap-2 sm:gap-1 min-h-[56px] sm:min-h-0 shadow-lg sm:shadow-md disabled:opacity-50 disabled:cursor-not-allowed ${className}`}
    >
      {isGenerating ? (
        <FiLoader className="w-5 h-5 sm:w-4 sm:h-4 animate-spin" />
      ) : (
        <FiDownload className="w-5 h-5 sm:w-4 sm:h-4" />
      )}
      <span className="truncate font-semibold sm:font-medium">
        {isGenerating ? 'Generating...' : 'Get Report'}
      </span>
    </motion.button>
  );
} 