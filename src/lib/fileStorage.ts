...
// Dynamic import for Sharp to handle Vercel serverless environment
let sharp: any = null;
...
const forceFallbackMode = process.env.SUPABASE_FALLBACK_MODE === 'true'; // Force fallback mode

// SUPABASE PRO: Create client with Pro tier optimizations
const supabase = (supabaseUrl && supabaseServiceKey && !forceFallbackMode) ? 
...
export class FileStorageService {
  ...
  static isSupabaseStorageAvailable(): boolean {
    try {
      // If fallback mode is forced, return false to use base64
      if (forceFallbackMode) {
        console.log('🔄 Supabase fallback mode enabled - using base64 storage');
        return false;
      }
      ...
      if (!hasValidConfig) {
        console.log('⚠️ Supabase not properly configured - using fallback storage mode');
      }
  ...
  static async testSupabaseConnection(timeoutMs: number = 5000): Promise<boolean> {
    ...
    try {
      ...
      // Simple bucket list test - no health table dependency
  ...
  static async uploadImage(
    ...
    if (isSupabaseAvailable) {
      try {
        ...
        if (!sharpLib) {
          console.warn('Sharp not available, falling back to unprocessed image');
          // Fall through to upload original image
          const fileName = `${Date.now()}-${file.name.replace(/\s/g, '_').toLowerCase()}`;
          const filePath = `public/${fileName}`;
        ...
        const processedImage = sharpLib.default(imageBuffer);
  ...
  static async uploadFile(
    ...
    if (isSupabaseAvailable) {
      try {
        // Simple direct upload - no complex chunking
        const fileName = `${Date.now()}-${file.name.replace(/\s/g, '_').toLowerCase()}`;
        const filePath = `public/${fileName}`;
    ...
    // FALLBACK: Simple base64 for files
    console.log('📦 Using base64 fallback for file...');
