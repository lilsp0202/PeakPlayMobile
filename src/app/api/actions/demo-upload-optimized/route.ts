...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Different size limits based on file type and storage method
    let maxSize: number;
    if (file.type.startsWith('image/')) {
      maxSize = storageInfo.recommendedUploadMethod === 'supabase' ? 
        20 * 1024 * 1024 : // 20MB for images with Supabase
        10 * 1024 * 1024;   // 10MB for images with base64 fallback
    } else {
      maxSize = storageInfo.recommendedUploadMethod === 'supabase' ? 
        100 * 1024 * 1024 : // 100MB for videos with Supabase
        25 * 1024 * 1024;   // 25MB for videos with base64 fallback
    }
    ...
    try {
      ...
      if (file.type.startsWith('image/')) {
        ...
        console.log('🎥 Processing demo video with optimized handling...');
        
        // PERFORMANCE: Streaming video upload with chunking
        uploadResult = await FileStorageService.uploadFile(
          file,
          'demo-media',
          {
            onProgress: progressCallback,
            // PERFORMANCE: Larger chunks for video files
            chunkSize: 10 * 1024 * 1024, // 10MB chunks for better performance
            // PERFORMANCE: Enable video-specific optimizations
            videoOptimizations: {
              enableThumbnail: true,
              thumbnailTimestamp: 1, // 1 second into video
              enableCompression: file.size > 50 * 1024 * 1024, // Only compress large videos
              targetBitrate: file.size > 100 * 1024 * 1024 ? '2M' : undefined // Compress very large videos
            }
          }
        );
      } else {
